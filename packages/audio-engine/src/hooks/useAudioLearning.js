...
export const useAudioLearning = () => {
    ...
    const learningEngineRef = useRef(null);
    ...
    const startAnalysis = (audioEngine, learningEngine) => {
        if (analysisIntervalRef.current) {
            window.clearInterval(analysisIntervalRef.current);
        }
    ...
    const predictNextPattern = async () => {
        if (!learningEngineRef.current || !audioEngineRef.current)
            return null;
    ...
    return {
        isInitialized,
        isPlaying: ((_a = audioEngineRef.current) === null || _a === void 0 ? void 0 : _a.isPlaying) || false,
        isLearning,
        error,
        toggleAudio,
        getStyleVector,
...
//# sourceMappingURL=useAudioLearning.js.map