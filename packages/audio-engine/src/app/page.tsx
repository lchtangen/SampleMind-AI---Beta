...
// Type for our audio ref with additional properties
          ...
          <div className="flex justify-between h-16">
            <div className="flex items-center space-x-8">
              ...
              <div className="hidden md:flex space-x-1">
                {['home', 'studio', 'library', 'templates'].map((tab) => (
                  <button
                    key={tab}
        ...
        <div className="text-center mb-16">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-6 mb-16">
            <button
              ...
              disabled={isPlaying}
              className={`px-8 py-4 rounded-xl font-medium flex items-center justify-center space-x-2 transition-opacity ${
                isRecording
            ...
            <button 
              className="px-8 py-4 border-2 border-cyan-500 text-cyan-400 rounded-xl font-medium flex items-center justify-center space-x-2 hover:bg-cyan-500/10 transition-colors"
              disabled={!audioBlob}
            >
            ...
            <div className="flex items-end justify-center h-full space-x-1">
              {audioData.length > 0 ? (
                Array.from(audioData).map((value, index) => (
                  <div
                    key={index}
                    className="w-1.5 bg-gradient-to-t from-cyan-400 to-blue-500 rounded-full transition-all duration-75"
                    style={{ height: `${value / 2}%` }}
                  />
                ))
              ) : (
                <div className="text-gray-500 text-lg">Start recording to see visualization</div>
              )}
            </div>
          </div>
          ...
          {/* Audio Player */}
          {audioUrl && (
            <div className="max-w-2xl mx-auto bg-gray-800/50 rounded-xl p-6 mb-12">
              <audio
                ref={audioRef}
                src={audioUrl}
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex-1 mx-4">
                  <input
                    ...
                    value={currentTime}
                    onChange={handleSeek}
                    className="w-full h-1 bg-gray-700 rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-cyan-500"
                  />
                </div>
