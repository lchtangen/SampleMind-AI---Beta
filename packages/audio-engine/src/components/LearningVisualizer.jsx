...
export const LearningVisualizer = ({ neuroplasticLearning, width = 400, height = 200, className = '' }) => {
    const canvasRef = useRef(null);
    const [isLearning, setIsLearning] = useState(false);
    const [learningProgress, setLearningProgress] = useState(0);
    const [styleVector, setStyleVector] = useState([]);
    ...
    useEffect(() => {
        ...
        if (!canvas || styleVector.length === 0)
            return;
    ...
    if (error) {
        return (<div className={`bg-gray-900 p-4 rounded-lg text-red-400 ${className}`}>
        Error: {error}
      </div>);
    }
    return (<div className={`relative ${className}`}>
      <canvas ref={canvasRef} width={width} height={height} className="w-full h-full rounded-lg border border-gray-700"/>
      <div className="absolute bottom-2 left-2 text-xs text-gray-400">
        Style Vector
      </div>
      {isLearning && (<div className="absolute top-2 right-2 text-xs bg-cyan-500 text-white px-2 py-1 rounded-full">
          LEARNING
        </div>)}
    </div>);
};
export default LearningVisualizer;
//# sourceMappingURL=LearningVisualizer.jsx.map