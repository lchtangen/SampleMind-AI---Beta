...
export const EnhancedVisualizer = ({ audioEngine, width = 800, height = 300, className = '' }) => {
    ...
    const drawWaveform = (ctx, analyser, width, height) => {
    ...
    const drawSpectrum = (ctx, analyser, width, height) => {
        const bufferLength = analyser.frequencyBinCount;
        const dataArray = new Uint8Array(bufferLength);
    ...
    const drawParticles = (ctx, analyser, width, height) => {
        const bufferLength = analyser.frequencyBinCount;
        const dataArray = new Uint8Array(bufferLength);
        analyser.getByteFrequencyData(dataArray);
    ...
    const drawNeuralNetwork = (ctx, analyser, width, height) => {
        const bufferLength = analyser.frequencyBinCount;
        const dataArray = new Uint8Array(bufferLength);
        analyser.getByteFrequencyData(dataArray);
    ...
    if (!isInitialized) {
        return (<div className={`${className} bg-gray-900/50 flex items-center justify-center`} style={{ width, height }}>
    ...
    return (<div className={`relative ${className}`} style={{ width, height }}>
      <canvas ref={canvasRef} width={width} height={height} className="w-full h-full rounded-lg"/>
      
      {/* Visualization mode selector */}
      <div className="absolute bottom-2 right-2 flex space-x-2">
        {['waveform', 'spectrum', 'particles', 'neural'].map((m) => (<button key={m} onClick={() => setMode(m)} className={`px-3 py-1 text-xs rounded-full transition-colors ${mode === m
...
//# sourceMappingURL=EnhancedVisualizer.jsx.map