...
> Consolidated from `audio_api.md` (archived 2026-03-07)

### Overview

The Audio Processing API provides advanced audio feature extraction and processing capabilities,
including tempo detection, key detection, spectral analysis, and harmonic/percussive source separation.

### Audio Features Endpoints

#### Extract Audio Features
- `POST /api/audio/features` — Extracts various audio features from the provided audio data.
...
- `POST /api/audio/separate` — Separates the harmonic and percussive components of an audio signal.
...
response = requests.post(
    ...
    json={'audio_data': audio_base64, 'sample_rate': sr, 'features': ['tempo', 'key', 'spectral', 'mfcc']}
)
