...
class PerformanceProfiler:
    ...
    def profile_semantic_search(self) -> Dict[str, float]:
        ...
        # Initialize ChromaDB
        init_chromadb(
            persist_directory="./data/chroma_perf", collection_name="perf_test"
        )
        ...
        for i in range(10):
            ...
            embedding = (
                extractor.generate_embedding(audio_array=varied_audio, sr=sr)
                if hasattr(extractor.generate_embedding, "__code__")
                and "audio_array" in extractor.generate_embedding.__code__.co_varnames
                else extractor.generate_embedding(Path(self.temp_dir) / f"test_{i}.wav")
            )
...
"""
        ...
        all_results = {}
        for results in [
            self.profile_audio_analysis(),
            self.profile_neural_embedding(),
            self.profile_semantic_search(),
        ...
        for op, time_taken in sorted(
            ...
            status = (
                "🟢 OK"
                if time_taken < 1.0
                else "🟡 SLOW" if time_taken < 3.0 else "🔴 VERY SLOW"
            )
