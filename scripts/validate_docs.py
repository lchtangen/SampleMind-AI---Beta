...
class DocumentationValidator:
    ...
    def validate_internal_links(self, file_path: Path, content: str) -> list[str]:
        ...
        for link in links:
            ...
            if not target_path.exists():
                errors.append(
                    f"Broken link: {link} (from {file_path.relative_to(self.project_root)})"
                )
    ...
    def validate_structure(self, file_path: Path, content: str) -> list[str]:
        ...
        if not content.strip().startswith("#"):
            errors.append(
                f"Missing main header: {file_path.relative_to(self.project_root)}"
            )
        ...
        if len(content.strip()) < 50:
            errors.append(
                f"Very short content: {file_path.relative_to(self.project_root)}"
            )
...
def main():
    ...
    if error_count == 0 and warning_count == 0:
        ...
        logger.error(
            f"❌ Documentation validation failed with {error_count} errors and {warning_count} warnings"
        )
