  ...
  const pricingPlans = [
    {
      ...
      buttonClass: "bg-muted text-foreground hover:bg-muted/80 neon-border transition-all duration-300 hover:neon-glow",
    },
    {
      name: "Pro",
      monthlyPrice: "$20",
      ...
      buttonText: "Join now",
      buttonClass: "bg-primary text-primary-foreground hover:bg-primary/90 neon-glow transition-all duration-300",
      popular: true,
    },
    {
      name: "Ultra",
      ...
      buttonClass: "bg-accent text-accent-foreground hover:bg-accent/90 neon-glow-green transition-all duration-300",
    },
  ]
    ...
    <section className="w-full px-5 overflow-hidden flex flex-col justify-start items-center my-0 py-8 md:py-14">
      <div className="self-stretch relative flex flex-col justify-center items-center gap-2 py-0">
        <div className="flex flex-col justify-start items-center gap-4">
          <h2 className="text-center text-foreground text-4xl md:text-5xl font-semibold leading-tight md:leading-[40px] neon-text">
        ...
        <div className="pt-4">
          <div className="p-0.5 glassmorphism rounded-lg neon-border flex justify-start items-center gap-1 md:mt-0">
            <button
              onClick={() => setIsAnnual(true)}
              className={`pl-2 pr-1 py-1 flex justify-start items-start gap-2 rounded-md transition-all duration-300 ${isAnnual ? "bg-primary/20 neon-glow" : ""}`}
            >
              <span
            ...
            <button
              ...
              className={`px-2 py-1 flex justify-start items-start rounded-md transition-all duration-300 ${!isAnnual ? "bg-primary/20 neon-glow" : ""}`}
            >
              <span
                className={`text-center text-sm font-medium leading-tight ${!isAnnual ? "text-foreground" : "text-muted-foreground"}`}
              >
      ...
      <div className="self-stretch px-5 flex flex-col md:flex-row justify-start items-start gap-4 md:gap-6 mt-6 max-w-[1100px] mx-auto">
        {pricingPlans.map((plan) => (
          <div
            key={plan.name}
            className={`flex-1 p-4 overflow-hidden rounded-xl flex flex-col justify-start items-start gap-6 glassmorphism neon-border transition-all duration-300 hover:neon-glow ${plan.popular ? "bg-primary/10" : ""}`}
          >
            <div className="self-stretch flex flex-col justify-start items-start gap-6">
              <div className="self-stretch flex flex-col justify-start items-start gap-8">
                <div className="w-full h-5 text-sm font-medium leading-tight text-foreground">
                  {plan.name}
                  {plan.popular && (
                    <div className="ml-2 px-2 overflow-hidden rounded-full justify-center items-center gap-2.5 inline-flex mt-0 py-0.5 bg-primary neon-glow">
                ...
                {plan.features.map((feature) => (
                  <div key={feature} className="self-stretch flex justify-start items-center gap-2">
                    <div className="w-4 h-4 flex items-center justify-center">
                      <Check className="w-full h-full text-accent" strokeWidth={2} />
                    </div>
