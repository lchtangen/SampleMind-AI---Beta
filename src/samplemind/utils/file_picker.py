...
class MacOSFilePicker:
    ...
    def choose_file(
        ...
        if initial_directory and initial_directory.exists():
            applescript_parts[
                2
            ] += f' default location alias POSIX file "{initial_directory}"'
...
class TkinterFilePicker:
    ...
    def choose_folder(
        ...
        try:
            ...
            dirname = filedialog.askdirectory(title=title, initialdir=initial_dir)
...
class CrossPlatformFilePicker:
    ...
    def choose_file_or_folder(
        ...
        """
        initial_dir = (
            Path(initial_directory).expanduser() if initial_directory else None
        )
                ...
                if result.returncode == 0 and result.stdout.strip():
                    ...
                    if "file" in choice:
                        return self.choose_file(
                            "Select File", initial_directory=initial_dir
                        )
                    elif "folder" in choice:
                        return self.choose_folder(
                            "Select Folder", initial_directory=initial_dir
                        )
    ...
    def choose_file(
        ...
        # Linux - Use ONLY Zenity (modern Ubuntu/GNOME native dialog)
            ...
            if file_types:
                file_filter = (
                    f"Audio Files|{'|'.join(['*.' + ext for ext in file_types])}"
                )
            else:
                file_filter = None
            ...
            if self.detector.has_zenity():
                return LinuxFilePicker.choose_file_zenity(
                    title, file_filter, initial_dir
                )
        ...
        # Windows - Use Tkinter
            ...
            if self.detector.has_tkinter():
                tk_filetypes = (
                    [("Audio Files", " ".join(["*." + ext for ext in file_types]))]
                    if file_types
                    else [("All Files", "*.*")]
                )
    ...
    def get_platform_info(self) -> dict:
        ...
        return {
            ...
            "desktop_env": (
                self.detector.get_desktop_environment() if self.os == "linux" else None
            ),
        }
...
def select_any_file(
    ...
    picker = get_file_picker()
    return picker.choose_file(
        title=title, file_types=None, initial_directory=initial_directory
    )
