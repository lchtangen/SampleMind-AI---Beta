...
class GenreClassifier:
    ...
    def classify(self, y: np.ndarray, sr: int) -> GenreResult:
        ...
        if self.use_deep_model:
            ...
            if deep_scores:
                ...
                for g in all_genres:
                    combined[g] = 0.6 * deep_scores.get(
                        g, 0.0
        ...
        sorted_genres = sorted(
            final_scores, key=lambda g: final_scores[g], reverse=True
        )
        ...
        result.primary_confidence = round(
            final_scores.get(result.primary_genre, 0.0), 4
        )
    ...
    def _deep_classify(y: np.ndarray, sr: int) -> tuple[dict[str, float], str]:
        ...
        try:
            ...
            scores = {normalize_genre(r["label"]): float(r["score"]) for r in results}
            return scores, "cnn"
        except (ImportError, Exception):
            pass
