...
class Settings(BaseSettings):
    ...
    jwt_secret_key: str = Field(
        default_factory=lambda: secrets.token_hex(32), env="JWT_SECRET_KEY"
    )
    ...
    session_secret_key: str = Field(
        default_factory=lambda: secrets.token_hex(32), env="SESSION_SECRET_KEY"
    )
    ...
    mongodb_url: str = Field(
        default="mongodb://localhost:27017/samplemind", env="MONGODB_URL"
    )
    ...
    email_verification_enabled: bool = Field(
        default=True, env="EMAIL_VERIFICATION_ENABLED"
    )
    email_verification_expire_hours: int = Field(
        default=24, env="EMAIL_VERIFICATION_EXPIRE_HOURS"
    )
    password_reset_expire_hours: int = Field(
        default=1, env="PASSWORD_RESET_EXPIRE_HOURS"
    )
    ...
    audio_max_duration_seconds: int = Field(
        default=1800, env="AUDIO_MAX_DURATION_SECONDS"
    )
