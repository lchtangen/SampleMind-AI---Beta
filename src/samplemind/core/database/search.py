...
class FullTextSearch:
    ...
    async def fuzzy_search(
        ...
        return [
            {"id": row.id, "value": getattr(row, field), "score": float(row.score)}
...
class VectorSearch:
    ...
    async def similarity_search(
        ...
        return [(row.audio_id, float(row.distance)) for row in result.fetchall()]
    ...
    async def batch_similarity_search(
        ...
        for idx, vector in enumerate(query_vectors):
            search_results = await VectorSearch.similarity_search(
                session, vector, limit=limit_per_query
            )
