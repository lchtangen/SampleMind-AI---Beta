...
class DatabaseBackup:
    ...
    async def create_backup(
        ...
        cmd = [
            ...
            db_info["database"],
            "-F",
            "c",  # Custom format (compressed, allows parallel restore)
            "-f",
            str(backup_file),
            "--verbose",
        ]
        ...
        try:
            ...
            logger.info(
                f"Backup created: {backup_file} ({self._get_file_size(backup_file)})"
            )
    ...
    async def restore_backup(
        self, backup_file: Path, database_url: str, drop_existing: bool = False
    ) -> bool:
        """
