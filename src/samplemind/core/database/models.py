...
class User(Base):
    ...
    collections = relationship(
        "AudioCollection", back_populates="user", cascade="all, delete-orphan"
    )
...
class AudioCollection(Base):
    ...
    __table_args__ = (Index("idx_collections_user_public", "user_id", "is_public"),)
...
class AuditLog(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    user_id = Column(
        String(100), ForeignKey("users.id", ondelete="SET NULL"), index=True
    )
...
class UserRepository:
    ...
    async def update_usage(
        session, user_id: str, uploads: int = 0, storage_mb: float = 0
    ):
