...
class AnalysisPreferences(BaseModel):
    ...
    auto_analyze_on_upload: bool = Field(
        default=False, description="Automatically analyze files after upload"
    )
    ...
    extract_all_features: bool = Field(
        default=True, description="Extract all audio features"
    )
    cache_analysis_results: bool = Field(
        default=True, description="Cache analysis results for reuse"
    )
...
class NotificationPreferences(BaseModel):
    ...
    email_on_analysis_complete: bool = Field(
        default=True, description="Email when analysis completes"
    )
    email_on_upload_error: bool = Field(
        default=True, description="Email on upload errors"
    )
    in_app_notifications: bool = Field(
        default=True, description="Show in-app notifications"
    )
    notification_sound: bool = Field(
        default=True, description="Play notification sounds"
    )
    digest_frequency: str = Field(
        default="daily", description="Email digest frequency: never, daily, weekly"
    )
...
class CloudSyncSettings(BaseModel):
    ...
    sync_frequency: int = Field(
        default=60, ge=30, le=3600, description="Sync frequency in seconds"
    )
    sync_library: bool = Field(default=True, description="Sync library metadata")
    sync_analysis_results: bool = Field(
        default=True, description="Sync analysis results"
    )
    ...
    backup_frequency: str = Field(
        default="daily", description="Backup frequency: never, daily, weekly"
    )
...
class PrivacySettings(BaseModel):
    ...
    data_retention_days: int = Field(
        default=365, ge=0, description="Days to retain analysis data (0=forever)"
    )
    ...
    share_feedback: bool = Field(
        default=False, description="Share usage feedback with developers"
    )
    profile_visibility: str = Field(
        default="private", description="Profile visibility: private, public"
    )
