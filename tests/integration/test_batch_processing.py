...
pytestmark = pytest.mark.skipif(
    not all(f.exists() for f in TEST_FILES), reason="Test audio files not found"
)
...
class TestBatchProcessing:
    ...
    def test_batch_with_invalid_files(self, processor, temp_dir):
        ...
        input_files = [TEST_FILES[0], temp_dir / "nonexistent.wav"]  # valid  # invalid

        # Process batch
        results = processor.process_batch(
            input_paths=input_files, output_dir=temp_dir / "with_errors"
        )
    ...
    def test_stop_processing(self, processor, temp_dir):
        ...
        def process():
            return processor.process_batch(
                input_paths=test_dir, output_dir=temp_dir / "stopped"
            )
