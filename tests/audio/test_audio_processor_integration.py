...
class TestAudioProcessorIntegration:
    ...
    def test_click_removal(self, processor, tmp_path):
        ...
        click_indices = np.random.choice(
            len(y), size=min(100, len(y) // 1000), replace=False
        )
        y_clicks[click_indices] = 0.5 * np.max(
            np.abs(y)
        )  # Add clicks at 50% of max amplitude
        ...
        assert (
            clean_energy < click_energy * 0.5
        )  # At least 50% reduction in click energy
