...
class TestNoiseReduction:
    ...
    def test_spectral_reduction(self, noisy_signal, clean_sine_wave):
        ...
        # Process the signal
        denoised = nr.reduce_noise(noisy_signal, method="spectral", reduction_db=12.0)
        ...
        if not os.environ.get("CI"):  # Don't save files in CI
            sf.write(
                TEST_OUTPUT_DIR / "test_spectral_original.wav",
                noisy_signal,
                SAMPLE_RATE,
            )
            sf.write(
                TEST_OUTPUT_DIR / "test_spectral_denoised.wav", denoised, SAMPLE_RATE
            )
    ...
    def test_wiener_reduction(self, noisy_signal):
        ...
        # Process the signal
        denoised = nr.reduce_noise(noisy_signal, method="wiener", reduction_db=12.0)
    ...
    def test_click_removal(self, clicky_signal):
        ...
        for method in ["median", "linear", "spline"]:
            cleaned = nr.remove_clicks(clicky_signal, threshold=0.1, method=method)
            ...
            if not os.environ.get("CI"):
                sf.write(
                    TEST_OUTPUT_DIR / f"test_clicks_{method}.wav", cleaned, SAMPLE_RATE
                )
    ...
    def test_de_essing(self, sibilant_signal):
        ...
        # Apply de-essing
        deessed = nr.de_ess(sibilant_signal, threshold=0.15, ratio=6.0)
        ...
        if not os.environ.get("CI"):
            sf.write(
                TEST_OUTPUT_DIR / "test_deess_original.wav",
                sibilant_signal,
                SAMPLE_RATE,
            )
...
class TestAudioEffectsIntegration:
    ...
    def test_effects_chain(self, clean_sine_wave):
        ...
        if not os.environ.get("CI"):
            sf.write(
                TEST_OUTPUT_DIR / "test_effects_original.wav",
                clean_sine_wave,
                SAMPLE_RATE,
            )
            sf.write(
                TEST_OUTPUT_DIR / "test_effects_processed.wav", processed, SAMPLE_RATE
            )
    ...
    def test_batch_processing(self, tmp_path):
        ...
        processor.add_effect(
            "pitch_shift", {"n_steps": -2.0}
        )  # Shift down a whole step
