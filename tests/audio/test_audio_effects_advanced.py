...
def generate_test_signal(
    duration=DURATION, freq=FREQ, sample_rate=SAMPLE_RATE, noise_level=0.0
):
...
class TestAudioEffectsProcessor:
    ...
    def test_time_stretch(self, processor):
        ...
        if not os.environ.get("CI"):
            ...
            sf.write(
                TEST_OUTPUT_DIR / "test_stretch_processed.wav", y_stretched, SAMPLE_RATE
            )
    ...
    def test_reverb_effect(self, processor):
        ...
        y_reverb = processor.apply_reverb(
            y, SAMPLE_RATE, room_size=0.8, damping=0.5, wet_level=0.5, dry_level=0.5
        )
        ...
        if not os.environ.get("CI"):
            ...
            sf.write(
                TEST_OUTPUT_DIR / "test_reverb_processed.wav", y_reverb, SAMPLE_RATE
            )
    ...
    def test_delay_effect(self, processor):
        ...
        y_delayed = processor.apply_delay(
            y, SAMPLE_RATE, delay_seconds=delay_time, feedback=0.5, mix=0.5
        )
        ...
        assert np.any(
            np.abs(peak_diff - delay_samples) < (0.05 * delay_samples)
        )  # Within 5%
        ...
        if not os.environ.get("CI"):
            ...
            sf.write(
                TEST_OUTPUT_DIR / "test_delay_processed.wav", y_delayed, SAMPLE_RATE
            )
    ...
    def test_compression_effect(self, processor):
        ...
        if not os.environ.get("CI"):
            ...
            sf.write(
                TEST_OUTPUT_DIR / "test_comp_compressed.wav", y_compressed, SAMPLE_RATE
            )
    ...
    def test_effect_chaining(self, processor):
        ...
        if not os.environ.get("CI"):
            ...
            sf.write(
                TEST_OUTPUT_DIR / "test_chain_processed.wav", y_processed, SAMPLE_RATE
            )
