...
class TestAnalysisRepository:
    ...
    async def test_create_analysis(self):
        ...
        analysis_data = {
            ...
            "user_id": "test_user_123",
            "analysis_type": "full",
            "results": {"bpm": 120, "key": "C", "scale": "major"},
            "processing_time": 2.5,
        }
...
class TestUserRepository:
    ...
    async def test_update_user_stats(self):
        ...
        with patch.object(repo, "update", new_callable=AsyncMock) as mock_update:
            updated_data = {"total_uploads": 10, "total_analyses": 8}
            mock_user = MagicMock()
...
class TestChromaDBOperations:
    ...
    async def test_add_embedding(self):
        ...
        with patch.object(
            chroma_client, "add_embedding", new_callable=AsyncMock
        ) as mock_add:
    ...
    async def test_query_similar(self):
        ...
        with patch.object(
            chroma_client, "query_similar", new_callable=AsyncMock
        ) as mock_query:
            mock_results = {
                "ids": [["audio_123", "audio_456"]],
                "distances": [[0.1, 0.3]],
                "metadatas": [
                    [{"filename": "similar1.wav"}, {"filename": "similar2.wav"}]
                ],
            }
            ...
            results = await chroma_client.query_similar(
                query_embedding=query_embedding, n_results=2
            )
