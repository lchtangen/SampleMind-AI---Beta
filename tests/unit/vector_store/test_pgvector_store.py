...
def test_db():
    ...
    # Use test database
    store = PgVectorStore(
        "postgresql://samplemind:samplemind123@localhost:5432/samplemind_test"
    )
...
class TestPgVectorStoreIntegration:
    ...
    def test_similarity_search(self, test_db):
        ...
        similar_embedding = base_embedding + np.random.normal(0, 0.1, 1536).astype(
            np.float32
        )
        different_embedding = np.random.rand(1536).astype(np.float32)

        # Add records
        test_db.add_audio_features(
            AudioFeatureRecord(
                id="similar_1",
        ...
        different_dist = next(
            (r["distance"] for r in results if r["id"] == "different_1"), float("inf")
        )
